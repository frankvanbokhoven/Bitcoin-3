...
using Bitcoin3.BouncyCastle.Crypto.Signers;
using Bitcoin3.BouncyCastle.Math;
using Bitcoin3.BouncyCastle.Math.EC;
using System;
using Bitcoin3.BouncyCastle.Math.EC.Custom.Sec;
...
namespace Bitcoin3.Crypto
{
	internal class ECKey
	{
		...
		static ECKey()
		{
			_Secp256k1 = Bitcoin3.BouncyCastle.Crypto.EC.CustomNamedCurves.Secp256k1;
		...
		{
			if(isPrivate)
				_Key = new ECPrivateKeyParameters(new Bitcoin3.BouncyCastle.Math.BigInteger(1, vch), DomainParameter);
			else
			{
		...
		public static ECKey RecoverFromSignature(int recId, ECDSASignature sig, uint256 message, bool compressed)
		{
			...
			var n = curve.N;
			var i = Bitcoin3.BouncyCastle.Math.BigInteger.ValueOf((long)recId / 2);
			...
			var e = new Bitcoin3.BouncyCastle.Math.BigInteger(1, message.ToBytes());
			...
			var eInv = Bitcoin3.BouncyCastle.Math.BigInteger.Zero.Subtract(e).Mod(n);
		...
		private static ECPoint DecompressKey(Bitcoin3.BouncyCastle.Math.BigInteger xBN, bool yBit)
		{
