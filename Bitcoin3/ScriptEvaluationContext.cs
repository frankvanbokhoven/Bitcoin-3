...
namespace Bitcoin3
{
	...
	public class ScriptEvaluationContext
	{
		...
		{
			ScriptVerify = Bitcoin3.ScriptVerify.Standard;
			SigHash = Bitcoin3.SigHash.Undefined;
...
0xDF,0xE9,0x2F,0x46,0x68,0x1B,0x20,0xA0
		...
		bool CheckSig(byte[] vchSig, byte[] vchPubKey, Script scriptCode, TransactionChecker checker, int sigversion)
		{
			...
			if(!pubkey.Verify(sighash, scriptSig.Signature))
			{
				...
				var R = 4;
				var S = 6 + nLenR;
				var newS = new Bitcoin3.BouncyCastle.Math.BigInteger(1, vchSig, S, nLenS);
				var newR = new Bitcoin3.BouncyCastle.Math.BigInteger(1, vchSig, R, nLenR);
		...
		{
			if(SigHash == Bitcoin3.SigHash.Undefined)
				return true;
