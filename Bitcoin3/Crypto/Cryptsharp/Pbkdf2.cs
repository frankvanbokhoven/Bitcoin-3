...
using Bitcoin3.BouncyCastle.Crypto;
using Bitcoin3.Crypto.Internal;
using System;
...
namespace Bitcoin3.Crypto
{
	...
	public class Pbkdf2 : Stream
	{
		...
		{
			Bitcoin3.Crypto.Internal.Check.Null("hmacAlgorithm", hmacAlgorithm);
			Bitcoin3.Crypto.Internal.Check.Null("salt", salt);
			Bitcoin3.Crypto.Internal.Check.Length("salt", salt, 0, int.MaxValue - 4);
			Bitcoin3.Crypto.Internal.Check.Range("iterations", iterations, 1, int.MaxValue);
		...
		{
			Bitcoin3.Crypto.Internal.Check.Range("count", count, 0, int.MaxValue);
		...
		{
			Bitcoin3.Crypto.Internal.Check.Range("derivedKeyLength", derivedKeyLength, 0, int.MaxValue);
		...
		{
			Bitcoin3.Crypto.Internal.Security.Clear(_saltBuffer);
			Bitcoin3.Crypto.Internal.Security.Clear(_digest);
			Bitcoin3.Crypto.Internal.Security.Clear(_digestT1);
		...
		{
			Bitcoin3.Crypto.Internal.Check.Bounds("buffer", buffer, offset, count);
			int bytes = 0;
